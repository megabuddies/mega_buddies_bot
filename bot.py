    ...
    try:
        ...
        if result.get("found", False):
            message_text = (
                ...
                keyboard=[
                    [InlineKeyboardButton(
                        "🔍 Проверить", callback_data="action_check")],
                ]
                ...
                # Устанавливаем флаг, чтобы знать, что следующее сообщение -
                # для проверки
                context.user_data['expecting_check']=True
                ...
                logger.debug(
                    f"Проверка значения в базе данных: '{value}' от пользователя {
                        user.id}")
                ...
                # Log the check event
                db.log_event(
                    "check_whitelist", update.effective_user.id, {
                        "value": value}, bool(
                        result.get(
                            "found", False)))
                ...
                # Create reply markup with buttons for next actions
                keyboard=[
                    [InlineKeyboardButton("🔄 Проверить другое значение",
                                          callback_data="action_check")],
                ...
                logger.debug(
                    f"Получено сообщение от пользователя {user_id}: '{text}'")
                logger.debug(
                    f"Текущие флаги пользователя: expecting_check={
                        context.user_data.get('expecting_check')}, expecting_add={
                        context.user_data.get('expecting_add')}, expecting_remove={
                        context.user_data.get('expecting_remove')}")
                ...
                # Handle conversation states with явным приоритетом для добавления и
                # удаления
                if context.user_data.get('expecting_add'):
                logger.debug(
                    f"Обработка сообщения для добавления в базу данных: '{text}'")
                ...
                logger.debug(
                    f"Обработка сообщения для удаления из базы данных: '{text}'")
                ...
                logger.debug(
                    f"Обработка сообщения для проверки в базе данных: '{text}'")
                ...
                logger.debug(
                    f"Обработка обычного сообщения как проверки в базе данных: '{text}'")
                ...
                message_text=(
                    f"*❌ Результат проверки*\n\n"
                    f"Нам жаль, {
                        user.first_name}, но введенного значения пока нет в BuddyWL.\n\n"
                ...
                logger.debug(
                    f"Button callback: {callback_data} from user {
                        update.effective_user.id}")

                # First, acknowledge the callback query to stop the "loading" state on the
                # button
                await query.answer()
                ...
                logger.debug(
                    f"User {
                        update.effective_user.id} pressed Check button")
                await show_check_menu(update, context)
                elif callback_data == "action_stats":
                logger.debug(
                    f"User {
                        update.effective_user.id} pressed Stats button")
                await show_stats_menu(update, context)
                elif callback_data == "action_links":
                logger.debug(
                    f"User {
                        update.effective_user.id} pressed Links/FAQ button")
                await show_links_menu(update, context)
                elif callback_data == "action_admin":
                logger.debug(
                    f"User {
                        update.effective_user.id} pressed Admin button")
                await show_admin_menu(update, context)
                elif callback_data == "back_to_main":
                logger.debug(
                    f"User {
                        update.effective_user.id} returned to main menu")
                await show_main_menu(update, context)
                elif callback_data == "menu_admin":
                logger.debug(
                    f"User {
                        update.effective_user.id} navigated to admin menu")
                await show_admin_menu(update, context)
                ...
                logger.debug(
                    f"Temporary file {
                        context.user_data['import_file_path']} deleted")
                except Exception as e:
                ...
                # For safety, redirect to main menu when an unknown callback is
                # received
                await show_main_menu(update, context)
                ...
                # Создаем приложение без использования rate_limiter с проблемными
                # параметрами
                application=(
                    Application.builder()
                    .token(token)
                ...
                # Add callback query handler - перемещено после ConversationHandler, но
                # перед MessageHandler
